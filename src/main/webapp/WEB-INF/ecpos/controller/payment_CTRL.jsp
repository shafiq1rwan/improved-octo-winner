<script>
	app.controller('payment_CTRL',function($scope, $http, $timeout, $location, $route,$routeParams) {
			...
			$scope.submitPayment = function() {
				if ($scope.paymentMethod == "QR") {
					if ($('#terminal').val() == "") {
						...
						$('#scan_qr_modal').on('shown.bs.modal',
								function() {
									$('#qr_content').focus();
								})
					}
					...
					if($scope.paymentMethod == "Cash"){
						...
						var paymentSocket = new WebSocket(
						"ws://localhost:8080/paymentSocket");
			...
			function getPaymentSocketMessage(responseData) {
				if (responseData.includes("[IPOS-CONTACT-BANK]"))
					return "contactingBank";
						...
						.includes("[IPOS-CARD-ENTER-PIN]"))
					return "enterPin";
				else if (responseData
						.includes("[IPOS-INSERT-CARD]"))
					return "insertCard";
			...
			function getIposResponseMessage(iposResponse) {
				...
				var jsonMessage = iposResponse.replace(
						"[IPOS-RESPONSE]", '');
				console.log(jsonMessage)
				var responseJSON = JSON.parse(jsonMessage);

				return responseJSON['responseCode'] === '00' ? 'Transaction Approved'
			...
			$scope.proceedToQRPayment = function() {
				if ($scope.qrContent === null || $scope.qrContent === "") {
					...
					$http.post("${pageContext.request.contextPath}/rc/transaction/submit_payment",jsonData).then(
									function(response) {
										if (response.data.response_code === "00") {
											$('#loading_modal').modal('hide');
											console.log("success")
