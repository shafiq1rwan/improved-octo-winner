...
; (function ($, window, document, undefined) {
    ...
    var function_keys = {
        backspace: {
            text: '<i class="far fa-backspace"></i>'
        },
        shift: {
            text: '<i class="fal fa-arrow-alt-up"></i>'
        },
        space: {
            text: '&nbsp;'
        },
    ...
    var layouts = {
        english: [
            ...
            ['numeric_switch', 'space', 'symbol_switch']
        ],
        numeric: [
            ['1', '2', '3', '4', '5', '6', '7', '8', '9', '0'],
            ['-', '/', ':', ';', '(', ')', '$', '&', '@', '"'],
            ['.', ',', '?', '!', "'", 'backspace'],
            ['character_switch', 'space', 'symbol_switch'],
        ],
        symbolic: [
            ['[', ']', '{', '}', '#', '%', '^', '*', '+', '='],
            ['_', '\\', '|', '~', '<', '>'],
    ...
    Plugin.prototype = {
        ...
        events: function () {
            ...
            shift_key.on('click', function () {
            	if (counter == 0) {
            		...
            		counter = 1;
            		shift_key.html('');
            		shift_key.html('<i class="fal fa-arrow-alt-from-top fa-rotate-180"></i>');
            	} else if (counter == 1) {
            		capslock = true;
                    counter = 2;
                    shift_key.html('');
            		shift_key.html('<i class="fas fa-arrow-alt-up"></i>');
            	} else if (counter == 2) {
            		me.toggleShiftOff();
                    capslock = false;
            		counter = 0;
        ...
        toggleShiftOff: function () {
            ...
            counter = 0;
    		shift_key.html('');
    		shift_key.html('<i class="fal fa-arrow-alt-up"></i>');
        },
